...
\section{Determining stable topological defects in spinor BECs}
...
\begin{table}
    ...
    and the corresponding topological defects/solitons they describe.}
\end{table}
...
\subsection{Homotopy groups for a spin-2 system}
...
\(\mathbb{Z}_{\hat{\vb{F}}, \tau}\) factor.
This arises from the double winding of the condensate phase, which can be seen
from Fig.~\ref{fig: spin-2-spherical-harmonics}a.
...
mass circulation, but instead carry a circulation of the condensate spin
...
The superfluid velocity for a scalar system with atomic mass \(M\)
...
As the loop is encircled, the phase changes as~\cite{Barenghi2016}
\begin{equation}
    \Delta \tau = \oint_C \nabla \tau \cdot \dd \ell,
\end{equation}
where \(\dd \ell \) is the line element of integration.
If \(\psi_0\) is the value of \(\psi \) at the starting point of the loop and
\(\psi_1\) is the value of \(\psi \) at the end point, then the
single-valuedness condition of the wave function implies that \(\Delta \tau =
2\pi n_w\), where \(n_w \in \mathbb{Z}\).
For \(n_w \neq 0\) a phase defect arises, where at a point in space the phase
simultaneously takes on every value.
Thus, to avoid this singularity, the density must vanish at this point.

For spinor BECs, a vanishing density is not always necessary to accommodate the
phase defect.
...
The superfluid velocity for a spinor system is given as~\cite{Kawaguchi2012}:

\subsection{Vortices in spin-1 systems}\label{sec: vortices-spin-1}
Here we analytically construct a wave function that corresponds to a HQV\@.
...
For the case of the ferromagnetic phase, the order parameter space is simply
\(\mathcal{M}_\text{FM}={\text{SO}(3)}_{\hat{\vb{F}}, \tau}\).
Now, different from the polar case, the first homotopy group is calculated
as~\cite{Kawaguchi2012} \(\pi_1(\mathcal{M}_\text{FM}) = {\mathbb{Z}}_2\),
where \({\mathbb{Z}}_2=\{0, 1\} \) is the two-element group, which corresponds
to two different line defects: singular and nonsingular.
...
then we have is what is known as a Mermin-Ho vortex.
...
to what is known as an Anderson-Toulouse vortex~\cite{Woelfle1990}.
...
One of the simplest defects present in the BN phase is a HQV\@.
Note that this is topologically distinct from that of the spin-1 polar case
...
As mentioned previously, the first homotopy group of the cyclic phase
(\(\mathbb{Z} \cross_h \text{T}_{\hat{\vb{F}}, \tau}\)) is also non-Abelian and
hence supports non-Abelian defects.
...
These vortices can be constructed from Eq.~\eqref{eq: C-2-spinor} by applying a
condensate phase and general spin rotation, where a \(1/3\) vortex is
constructed from the choice \(\tau = -\alpha = \varphi/3 \) with \(\gamma = 0\)
and a \(2/3\) vortex is constructed by choosing \(\tau = 2\varphi/3\),
