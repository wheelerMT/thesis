...
Kawaguchi2012}.
To do so, one needs to first find the relevant order parameter space for the
given system, \(\mathcal{M}\), that describes the symmetries of that
particular order parameter.
...
space.
For example, given an order parameter space \(\mathcal{M}\), calculating the
second homotopy group \(\pi_2(\mathcal{M})\) can tell us whether point defects
are topologically stable within systems that have an order parameter space of
\(\mathcal{M}\).
...
\begin{table}
    ...
    \begin{tabular}{ccccc}
        ...
        \midrule
        Polar phase & \(\left[S^2_{\hat{\vb{F}}} \times {U(1)}_\tau\right]
        / {({\mathbb{Z}}_2)}_{\hat{\vb{F}}, \tau} \)
        & \(\mathbb{Z}\) & \(\mathbb{Z}\) & \(\mathbb{Z}\) \\
        Ferromagnetic phase & \({\text{SO}(3)}_{\hat{\vb{F}}, \tau}\)
        & \({\mathbb{Z}}_2\) & 0 & \(\mathbb{Z}\) \\
        \bottomrule
    \end{tabular}
    \caption{\label{tab: spin-1-homotopy-groups}Spin-1 phases and their relative
    order parameter space, \(\mathcal{M}\).
    Also given are the first three homotopy groups calculated from the
    corresponding order parameter space.
    Here, \(\hat{\vb{F}}\) and \(\tau \) indicate contributions from the
    condensate spin and phase, respectively.
    Additionally, \({\mathbb{Z}}_2=\{0, 1\} \) denotes the two-element group.}
\end{table}
...
\times {U(1)}_\tau\right] / {({\mathbb{Z}}_2)}_{\hat{\vb{F}}, \tau} \), where
