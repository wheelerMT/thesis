...
\begin{table}
    ...
    and the corresponding topological defects they describe.}
\end{table}
...
The order parameter space for this phase is given as~\cite{Kawaguchi2012}
\(\mathcal{M}_\text{polar} =\left[S^2_{\hat{\vb{F}}} \times {U(1)}_\tau\right]
/ {(\mathbb{Z})}_{\hat{\vb{F}}, \tau} \).
\begin{table}
    \centering
    \begin{tabular}{ccccc}
        \toprule
        Ground state & \(\mathcal{M}\) & \(\pi_1\) & \(\pi_2\) & \(\pi_3\) \\
        \midrule
        ...
        \bottomrule
    \end{tabular}
    \caption{\label{tab: spin-1-homotopy-groups}Caption.}
\end{table}
The first homotopy group is calculated to be \(\pi_1(\mathcal{M}_\text{polar})
= \mathbb{Z}\), which indicates vortices are stable within this phase.
