...
\begin{equation}
    \tau_s(\epsilon) \sim \xi^z \sim |\epsilon|^{-\nu z},
...
The equilibrium relaxation time \( \tau_s \) describes the time it takes for the
...
\begin{equation}
    \tau_s \approx |\epsilon/\dot{\epsilon}|=\tilde{t}.
...
In this case, the relaxation time scales as \(\tau_s \sim 1/\Delta \), which
...
integrator~\cite{Symes2016}.
We measure length and time in units of the spin healing length
\(\xi_s=\hbar/\sqrt{2M|c_1|n_0}\) and the spin time \(\tau_s =\hbar/2|c_1|n_0\),
respectively.
