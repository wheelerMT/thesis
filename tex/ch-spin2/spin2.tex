...
\begin{figure}
    ...
    \caption{\label{fig: C-FM-analytical-spin-singlet}Analytical plot
        goes here.}
\end{figure}
...
\begin{figure}
    ...
    \caption{\label{fig: C-FM-coreless-initial-states}Spherical harmonic
        ...
        (a) and (b): Coreless vortex in the FM-2 and FM-1 limit, respectively, given
        by Eq.~\eqref{eq: C-FM-coreless-FM-limits}.
...
\begin{equation}\label{eq: C-FM-DQV-cyclic-limit}
    \zeta^\text{DQV}
    = (\zeta^\text{|F|=2}_\text{cl} + \sqrt{2}\zeta^\text{|F|=1}_\text{cl}) / 3.
\end{equation}
