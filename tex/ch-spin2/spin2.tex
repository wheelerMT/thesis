...
\begin{gather}\label{eq: a20definition}
    |A_{00}|^2 = \frac{1}{5}\left|2\zeta_2\zeta_{-2}-2\zeta_1\zeta_{-1}
    + \zeta_0^2\right|^2, \\
...
\begin{equation}
    \begin{aligned}
        |A_{00}|^2 & = \frac{1}{10} \left[(\eta^2-1)\cos\chi
        + \eta^2 + 1\right],                                          \\
...
Fig.~\ref{fig: UN-BN-duo-trio} in a parameter space of \((\eta, \chi)\).
\begin{figure}
    \centering
    \includegraphics[width=0.8\textwidth]{gfx/ch-spin2/a20-a30-varying.png}
    \caption[Spin-singlet duo and trio amplitudes for a UN-BN interface in a
    parameter space of \(\chi \) and \(\eta \)]
...
difference \(\chi \).
For example, if one were to maintain \(\chi=0\) and interpolate \(\eta \),
...
BN (\(\eta = -1\)) limits we now have three-component spinors given,
respectively, as
\begin{align}
...
and \(\chi_{-2}=\varphi\), which connects it to a vortex-free state in the UN
limit.
...
constructed by thinking about a monopole placed in the UN side of the
interface.
...
\begin{gather}
    ...
    \mqty(
    -i\sin^2 \varphi \\
    -e^{-\frac{i\pi }{4}} i\sin 2\varphi \\
    \frac{1}{\sqrt{6}}\left(1 + 3\cos 2\varphi \right)\\
...
\subsection{Cyclic to ferromagnetic}\label{sec: spin-2-C-FM}
...
\end{align}
where \(\eta \) now becomes the longitudinal magnetisation,
\(\langle\hat{F}_z\rangle\).
At zero magnetisation, \(\langle\hat{F}_z\rangle = 0\), equivalent to setting
\(\eta=0\), this solution becomes the two-component cyclic state given in
...
\(\langle\hat{F}_z\rangle = 0\) and \(\langle\hat{F}_z\rangle = 2\).
...
depending on the value of the condensate spin, \(\langle\hat{F}_z\rangle\).
...
E^\text{C}=0, E^{\text{FM}_1} = c_1n/2 + p\).
...
the FM state with spin \(\langle\hat{F}_z\rangle = \pm 2\).
...
integrator~\cite{Symes2017a} (see Appendix~\ref{sec: symplectic-integrators} for
details on the numerical integration scheme) with, for simplicity, a purely
isotropic trapping potential \(V=M\omega^2r^2/2\).
...
We choose this as the initial state and choose
