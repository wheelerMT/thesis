...
across the topological interface.
A summary of the possible phase and spin vortex connections across a UN-BN
interface are listed in Table~\ref{tab: UN-BN-vortices}.
\begin{table}
    \centering
...
In the FM-2 and FM-1 cases, the order-parameter is kept single-valued by a
combined winding of the condensate phase coupled to a winding of the spin
vector, achieved, respectively, by
\begin{align}\label{eq: coreless-euler-limits}
    \tau - 2\gamma &= \pm 2\alpha = \pm 2\varphi \\
    \tau + \gamma &= \pm\alpha=\pm\varphi 
\end{align}
...
\begin{figure}
    ...
    (a) Nonsingular vortex in the FM-2 limit given by
    Eq.~\eqref{eq: FM-2-coreless} with the choice \(\tau - 2\gamma = 2\alpha
    = 2\varphi \).
    (b) Nonsingular vortex in the FM-1 limit given by
...
on the Euler angles given in Eq.~\eqref{eq: coreless-euler-limits}.
...
\subsubsection{Connections involving monopoles and nonsingular textures}
We can also construct nonsingular defect crossing solutions using this
interface.
...
connecting to a singular phase vortex in the FM-2 phase using
...
One can use the general spinor in Eq.~\eqref{eq: C-FM-third-pv} to
