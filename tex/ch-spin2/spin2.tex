...
the UN \(\zeta^\text{UN}_\text{pv} = {(0,0,e^{i\varphi},0,0)}^T\) and BN
\(\zeta^\mathrm{BN}_\text{pv} ={(e^{i\varphi},0,0,0,e^{i\varphi})}^T/\sqrt{2}\)
...
Nonsingular vortices arise in both the UN and BN limits, which carry zero
...
splitting process, whereby the SQV is expected to split into two
