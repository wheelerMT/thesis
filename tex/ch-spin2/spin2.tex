...
It is also possible to design a topological interface to divide a continuous
superfluid into spatially distinct bulk regions with various order parameter
symmetries.
This can be done, for instance, by spatially varying the Hamiltonian's
parameters, causing the condensate to seek a different ground state in each
region.
If the parameter fluctuation is sufficiently acute, then the wave function will
interpolate between the bulk phases across a distance given by the appropriate
healing length.
...
/ \delta \zeta_m^*=0\).
This yields~\cite{Kawaguchi2012}
\begin{equation}\label{eq: spin-2-GPEs-time-reversal}
    \left[-p \hat{F}_z + q \hat{F}^2_z + nc_0 \, \zeta^\dagger \zeta
        + nc_1 \, \langle\hat{\vb{F}}\rangle\cdot\hat{\vb{F}}
        + \frac{nc_2}{5}{(\hat{\mathcal{T}}\zeta)}^\dagger \,
...
Defect states for a given spin-2 phase can, in principle, be constructed from
the representative spinor by defining an appropriate azimuthal phase winding,
\(\varphi \), in each component, i.e., \(\text{Arg}(\zeta_m) =l_m\varphi \),
\(l_m \in \mathbb{Z}\).
Here, however, we construct a unitary framework which is applicable for
constructing defect states across topological interfaces in a spin-2 BEC.
A defect state can be constructed from a uniform state by applying a global
phase winding, \(\tau \), coupled with a spin rotation defined by three Euler
angles \((\alpha, \beta, \gamma)\) as in Eq.~\eqref{eq: spin-2-rotation-matrix}.
When the same transformation is supported by two phases, A and B, of a spin-2
system, a defect connection across an interface between the two phases is given
as
\begin{align}\label{eq: general-defect-interface}
    \zeta = e^{i\tau}\,\hat{U}(\alpha, \beta, \gamma)\,\zeta^\text{A-B}.
\end{align}

Through the subsequent sections we provide solutions that interpolate between
different phases of a spin-2 BEC.\@
Detailed derivations of each interpolating solution can be found in
...
phases.
Such an interpolating solution comes from considering the case \(D_1 \neq 0\)
and \(D_2 = 0\).
In such a case, \(\zeta_1 = \zeta_{-1} = 0\), and, if \(\tilde{\mu} = \alpha \),
then all of \(\zeta_{\pm 2}\) and \(\zeta_0\) can be non-zero.
...
singlet-duo and -trio amplitudes, which for this interface become
