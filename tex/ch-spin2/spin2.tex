...
We choose this as the initial state, with \(\beta = \pi(1 + \tanh(\rho-1))/2\)
to model the required monotonically increasing function.
Here we focus only on the cyclic to FM-2 limit.
\begin{figure}
    \centering
    \begin{tikzpicture}
        \node[inner sep=0pt] (initial_dqv) at (0, 0)
        {\includegraphics[width=0.3\textwidth]
            {gfx/ch-spin2/C-FM=2_coreless_cyclic_init_spherical.pdf}};
        \node[cylinder, draw, fill=black, opacity=0.5, minimum height=2.8cm,
            rotate=90] (c_dqv) at (0, 0) {};
        % \draw [-stealth] (-5, 0) -- (0, 0);
        \node[inner sep=0pt] (spinmag_dqv) at (5, 0)
        {\includegraphics[width=0.3\textwidth]
            {gfx/ch-spin2/C-FM=2_coreless_spinMag_cyclic.pdf}};
